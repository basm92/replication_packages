...
knitr::opts_current$set(label = "covbal_firststint")

datasummary(data = firstrents_pooled,
            align = c("llllllll"),
            formula = 
              ...
              (`p-val.` = p_val_close) + (`RD Estimate (SD)`=get_coef_and_se5), 
            out = "kableExtra",
            output = "latex",
            title = "Covariate Balance - First Term") %>%
  ...
  kableExtra::save_kable("./tables_main/covariate_balance_firststint.tex")
