...
panel_a <- data.frame(names = c("Coefficient (ITT)", 
                                ...
                                "N (Non-Politicians)",
                                "Bandwidth"),
                      an_defw = get_info(firstrents_firsttry, 'defw', covs = NULL),
                      an_defw_w = get_info(firstrents_firsttry, 'defw', bw_mult = 2),
                      an_defw_cov = get_info(firstrents_firsttry, 'defw', covs = covariates),
                      an_defw2_cov = get_info(firstrents_firsttry, 'defw', covs = covariates, bw_mult = 2),
                      an_defw_sec_cov = get_info(firstrents_secondtry, 'defw', covs = covariates3),
                      an_defw2_sec_cov = get_info(firstrents_secondtry, 'defw', covs = covariates3, bw_mult = 2),
                      an_defw_pooled_cov = get_info(firstrents_pooled, 'defw', covs = covariates2),
                      an_defw2_pooled_cov = get_info(firstrents_pooled, 'defw', covs = covariates2, bw_mult = 2))

notitie <- "Table showing Bias-corrected standard errors clustered at the individual-level. The first two columns show univariate regressions under the optimal MSE bandwidth, and twice the optimal bandwidth. In columns 3 and 4, selected covariates are added, in particular, covariates that seemed to be unbalanced at the 2\\\\% cutoff. In particular, the regression controls for birthplace population, birthplace characteristics, age at election, and socialist recommendations. In addition, I control for politicians' lifespan. Columns 5 and 6 focus on second-triers and columns 7 and 8 pool all attempts. *: p < 0.10, **: p < 0.05, ***: p < 0.01. "
...
datasummary_df(panel_a %>%
               ...
               out = "kableExtra",
               output = "latex",
               title = "Main RD Estimates - 1st Term") %>%
  ...
  kableExtra::save_kable("./tables_main/rdd_mainresults_firststint.tex")
