...
find_candidatenextelection <- function(row){
...
dataset <- dataset %>%
  ...
  mutate(candidacy_nextelection = find_candidatenextelection(row)) %>%
  unnest(cols = c(row))
...
model1 <- lm(data = firstrents_pooled, candidacy_nextelection ~ defw + party_category + age_at_election   + no_candidates + as.numeric(opkomst) + district)
model2 <- lm(data = secondrents, candidacy_nextelection ~ defw + party_category + age_at_election   + no_candidates + as.numeric(opkomst) + district)
model3 <- lm(data = thirdrents, candidacy_nextelection ~ defw + party_category + age_at_election  + no_candidates + as.numeric(opkomst) + district)
model4 <- lm(data = fourthrents, candidacy_nextelection ~ defw + party_category + age_at_election + no_candidates + as.numeric(opkomst) + district)
model5 <- lm(data = fifthrents, candidacy_nextelection ~ defw + party_category + age_at_election  + no_candidates + as.numeric(opkomst) + district)
model6 <- lm(data = sixthrents, candidacy_nextelection ~ defw + party_category + age_at_election  + no_candidates + as.numeric(opkomst) + district)
...
knitr::opts_current$set(label = "wealth_prob_candidacy")
...
modelsummary(models,
             ...
             title="Correlation between Wealth and Probability of Candidacy",
             ...
             out="kableExtra",
             output = "./tables_appendix/wealth_prob_candidacy.tex"
) %>%
  kableExtra::kable_styling(latex_options = c("hold_position", "scale_down"))  %>%
  kableExtra::footnote(general = notes, footnote_as_chunk = T, threeparttable = T, escape = F)  %>%
  kableExtra::save_kable("./tables_appendix/wealth_prob_candidacy.tex")
