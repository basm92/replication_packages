# helper: find_demographics_tk

## Return: dataframe with all demographic variables

find_demographics_tk <- function(df_tk) {
    
    career <- read_excel("./data/polid_data/tk_1815tot1950uu.xlsx", sheet = 2)
    ...
    together <- left_join(df_tk, career[[1]],
                          by = "b1_nummer"
                          ) %>% # Birthdate
